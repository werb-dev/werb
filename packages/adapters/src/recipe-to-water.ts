 ...
 *                               amount (if present), else default 3.0 L/kg
 *
 * Equipment-derived parameters (grain_absorption, dead spaces, evap rate,
 * shrinkage, kettle→fermenter loss) are NOT carried in BeerJSON's recipe.
 * The screen must combine the recipe with an equipment profile to get
 * realistic numbers — for now, callers can pass overrides via the
 * `equipment` argument.
 */
